...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const XML_PATH_SUBSCRIPTION = 'dibs_easycheckout/subscription/';
    ...
    /**
     * This function returns a subscription End Date
     *
     * @param $interval
     * @param $intervalTime
     * @return mixed
     */
    public function getSubscriptionEndDate($interval, $intervalTime) {
        $intervalData = explode("-", $interval);
        $intervalDay = $intervalData[0];
        $intervalFrequency = $intervalData[1];
        $subscriptionEndDate = date('Y-m-d', strtotime(date('2099-12-31')));
        if(!empty($intervalDay) && !empty($intervalFrequency) && $intervalTime !== "all time") {
            switch ($intervalDay) {
                case "Every" :
            ...
            $subscriptionDataInterval = "+" . ($subscriptionInterval * $intervalTime) . " " . $subscriptionFrequency;
            $subscriptionEndDate = date('Y-m-d', strtotime($subscriptionDataInterval, strtotime(date('Y-m-d'))));
        }
     ...
     * @return bool
     */
    public function isSubscriptionEnabled($store = null)
    {
        return $this->scopeConfig->isSetFlag(
            self::XML_PATH_SUBSCRIPTION . 'subscription_enabled',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $store
        );
    }
     ...
     * @return mixed
     */
    public function beforAddtoCartError($store = null)
    {
        return $this->scopeConfig->getValue(
            self::XML_PATH_SUBSCRIPTION . 'subscription_error_msg',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $store
        );
    }
     ...
     * @return mixed
     */
    public function afterAddtoCartError($store = null)
    {
        return $this->scopeConfig->getValue(
            self::XML_PATH_SUBSCRIPTION . 'subscription_error_after',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE,
            $store
        );
    }
