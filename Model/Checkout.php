...
class Checkout extends \Magento\Checkout\Model\Type\Onepage {
      ...
      /*  if (!$billingAddress->getCountryId() || $billingAddress->getCountryId() != $shippingAddress->getCountryId()) {
        ...
        if ($quote->getItemsCount() <= 1) {
            ...
            foreach ($products as $product) {
                ...
                $subscriptionEndDate = $product->getProduct()->getData('subscription_end_date');
            }
            
            if ($isSubscription && $this->getHelper()->isSubscriptionEnabled()) {
                $additionalData = $options['info_buyRequest'];
                if (isset($additionalData['nets-subscription']) && $additionalData['nets-subscription'] == 'Subscription Order') {
                    $subscriptionEndDate = $additionalData['subscription-end-date'];
                    $subscriptionInterval = $additionalData['intervals'];
                }
                $quote->setNetsSubscriptionInterval(str_replace("-", " ", $subscriptionInterval));
            		$quote->setNetsSubscriptionSpecificInterval($subscriptionIntervalTime);
            		$quote->setNetsSubscriptionEnddate($this->getHelper()->getSubscriptionEndDate($subscriptionInterval, $subscriptionIntervalTime));
                if (!empty($product->getProduct()->getData('signup_fee'))) {
                    $quote->setNetsSubscriptionSignupFee($product->getProduct()->getData('signup_fee'));
                    $quote->setNetsSubscriptionSignupName($product->getProduct()->getData('signup_fee_name'));
                }
            }
        }
    ...
    public function checkAndChangeCurrency() {
        ...
        if (!$country) {
            $quote->getBillingAddress()->setCountryId($this->getDefaultCountryFromConfiguration($quote->getStoreId()));
    ...
    public function placeOrder(GetPaymentResponse $dibsPayment, Quote $quote) {
        ...
        if ($quote->getItemsCount() == 1) {
            ...
            foreach ($products as $product) {
                ...
                $signUpFeeName = $product->getProduct()->getData('signup_fee_name');
                $subscriptionInterval = $product->getProduct()->getData('nets_interval') . "-" . $product->getProduct()->getData('nets_sub_interval');
                $subscriptionIntervalTime = $product->getProduct()->getData('nets_sub_interval_time');
                $subscriptionEndDate = $product->getProduct()->getData('subscription_end_date');
                $options = $product->getProduct()->getTypeInstance(true)->getOrderOptions($product->getProduct());
        ...
        try {
            ...
            if ($isSubscription && $this->getHelper()->isSubscriptionEnabled()) {
                ...
                $order->setNetsSubscriptionInterval(str_replace("-", " ", $subscriptionInterval));
                $order->setNetsSubscriptionSpecificInterval($subscriptionIntervalTime);
                $order->setNetsSubscriptionEnddate($this->getHelper()->getSubscriptionEndDate($subscriptionInterval, $subscriptionIntervalTime));
                if (!empty($product->getProduct()->getData('signup_fee'))) {
                    $order->setNetsSubscriptionSignupFee($product->getProduct()->getData('signup_fee'));
                    $order->setNetsSubscriptionSignupName($product->getProduct()->getData('signup_fee_name'));
                }
                $order->save();
            }
