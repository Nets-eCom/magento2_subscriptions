...
class Items
{
     ...
     * @param $quote
     */
    private function addCustomTotals($quote)
    {
     ...
     * @param $order
     */
    public function addSubscriptionFeeItem($order) {
        $item = $this->generateSubscriptionFeeItem($order);
        $this->addToCart($item);
    }
     ...
     * @return OrderItem
     */
    public function generateSubscriptionFeeItem($order) {
        $subscriptionFeeLabel = $order->getNetsSubscriptionSignupName();
        ...
        $taxAmount = $this->getTotalTaxAmount($subscriptionFee, $vat);
        $subscriptionFee = $this->addZeroes($subscriptionFee);
        $amountExclTax = $subscriptionFee - $taxAmount;

        $feeItem = new OrderItem();
        $feeItem
                ->setName($subscriptionFeeLabel)
                ->setReference("fee")
                ->setUnit("st3")
                ->setQuantity(1)
                ->setTaxRate($this->addZeroes($vat))
                ->setTaxAmount($taxAmount)
                ->setUnitPrice($amountExclTax)
                ->setNetTotalAmount($amountExclTax)
                ->setGrossTotalAmount($subscriptionFee);
        return $feeItem;
    }
