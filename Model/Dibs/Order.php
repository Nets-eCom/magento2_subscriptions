    ...
    protected function createNewDibsPayment(Quote $quote, $checkoutInfo) {
        ...
        if ($quote->getItemsCount() <= 1) {
            ...
            foreach ($products as $product) {
                ...
                $subscriptionInterval = $product->getProduct()->getData('nets_interval');
                $subscriptionEndDate = $product->getProduct()->getData('subscription_end_date');
            }
            if ($isSubscription && $this->helper->isSubscriptionEnabled()) {
                /* $additionalData = $options['info_buyRequest'];
                  ...
                  if ($subscriptionInterval == 3) {
                  $interval = 30;
                  }
                  if ($subscriptionInterval == 4) {
                  $interval = 90;
                  }
                  if ($subscriptionInterval == 5) {
                  $interval = 180;
                  }
                  if ($subscriptionInterval == 6) {
                  $interval = 365;
                  }
                  if ($subscriptionInterval == 7) {
                  $interval = 30; //$additionalData['specific-date'];
                  } */
                $unscheduledSubscriptionOrder = new CreateUnscheduledSubscriptionOrder();
                $unscheduledSubscriptionOrder->setCreate("true");
...
//EVENT_PAYMENT_CHARGE_CREATED
    ...
    public function captureDibsPayment(\Magento\Payment\Model\InfoInterface $payment, $amount) {
        ...
        if ($paymentId) {
            ...
            //Generate Items for Subscription Fee if enabled
            $order = $payment->getOrder();
            if ($order->getNetsSubscriptionSignupFee() > 0) {
                $this->items->addSubscriptionFeeItem($order);
            }
